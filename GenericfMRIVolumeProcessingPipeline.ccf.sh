...
# additional SLOMOCO 
if [ $SLOMOCOHCPDIR != "" ]; then
    log_Msg "mkdir -p ${fMRIFolder}/SLOMOCO"
    mkdir -p ${fMRIFolder}/SLOMOCO

    ${RUN} "$SLOMOCOHCPDIR"/slomoco.sh \
        "$fMRIFolder"/"${tcsEchoesGdc[0]}" \
        "$fMRIFolder"/SLOMOCO/"${tcsEchoesGdc[0]}_slomoco" \
        "$fMRIFolder"/"${sctEchoesGdc[0]}" \
        "$fMRIFolder"/"${sctEchoesGdc[0]}_mask" \
        "$fMRIFolder"/"$MotionMatrixFolder" \
        "$fMRIFolder"/SLOMOCO \
        ${SLIACQTIME} 
fi
...
# additional SLOMOCO onesampling
if [ ${SLOMOCOHCPDIR} != "" ] ; then
    log_Msg "One Step Resampling for SLOMOCO"
    log_Msg "mkdir -p ${fMRIFolder}/OneStepResampling_SLOMOCO"
    mkdir -p ${fMRIFolder}/OneStepResampling_SLOMOCO
    tscArgs="";sctArgs="";
    for iEcho in $(seq 0 $((nEcho-1))) ; do
        ${RUN} ${PipelineScripts}/OneStepResampling_SLOMOCO.sh \
        --workingdir=${fMRIFolder}/OneStepResampling_SLOMOCO \
        --infmri="${fMRIFolder}/${tcsEchoesOrig[iEcho]}.nii.gz" \
        --t1=${AtlasSpaceFolder}/${T1wAtlasName} \
        --fmriresout=${FinalfMRIResolution} \
        --fmrifolder=${fMRIFolder} \
        ...
        --motionmatdir=${fMRIFolder}/${MotionMatrixFolder} \
        --motionmatprefix=${MotionMatrixPrefix} \
        --ofmri="${fMRIFolder}/${tcsEchoesOrig[iEcho]}_slomoco_nonlin" \
        --freesurferbrainmask=${AtlasSpaceFolder}/${FreeSurferBrainMask} \
        --biasfield=${AtlasSpaceFolder}/${BiasFieldMNI} \
        --gdfield=${fMRIFolder}/${NameOffMRI}_gdc_warp \
        --scoutin="${fMRIFolder}/${sctEchoesOrig[iEcho]}" \
        ...
        --fmrirefpath=${fMRIReferencePath} \
        --fmrirefreg=${fMRIReferenceReg} \
        --slomocomotionmatdir=${fMRIFolder}/SLOMOCO/inplane/MAT \
        --sliacqfile=${SLIACQTIME}  \
        --wb-resample=${useWbResample}
...
if [ $SLOMOCOHCPDIR != "" ] ; then
    log_Msg "Intensity Normalization and Bias Removal for SLOMOCO output"
    ${RUN} ${PipelineScripts}/IntensityNormalization.sh \
...
if [[ ${nEcho} -gt 1 ]]; then
    ...
    echo ${RUN} cp ${fMRIFolder}/${NameOffMRI}_nonlin_norm.nii.gz ${ResultsFolder}/${NameOffMRI}.nii.gz
    ${RUN} cp ${fMRIFolder}/${NameOffMRI}_nonlin_norm.nii.gz ${ResultsFolder}/${NameOffMRI}.nii.gz
    if [ $SLOMOCOHCPDIR != "" ] ; then
        echo ${RUN} cp ${fMRIFolder}/${NameOffMRI}_slomoco_nonlin_norm.nii.gz ${ResultsFolder}/${NameOffMRI}_slomoco.nii.gz 
        ${RUN} cp ${fMRIFolder}/${NameOffMRI}_nonlin_norm.nii.gz ${ResultsFolder}/${NameOffMRI}.nii.gz
    fi
